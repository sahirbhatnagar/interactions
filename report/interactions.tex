...
\hlstd{main_effect_names} \hlkwb{<-} \hlkwd{paste0}\hlstd{(}\hlstr{"x"}\hlstd{,} \hlnum{1}\hlopt{:}\hlstd{p)}  \hlcom{# names of the main effects}
...
\hlcom{# simulate Toeplitz like structure}
...
## Subject1 -0.16 -0.92 -1.54 -0.79 -1.04 -0.148  0.257 -0.28
## Subject2  0.37  0.28  0.36 -0.69  0.60 -0.151  0.011  0.73
## Subject3  0.71  1.10  1.26 -0.19 -0.83  0.323 -0.223  1.16
## Subject4 -0.44  0.66  0.44  0.50  0.33  0.025 -1.229 -0.99
## Subject5  1.30  0.15 -0.82 -1.04 -1.32 -1.796 -0.782 -1.47
## Subject6 -0.82 -1.71  0.93  0.87 -0.09 -0.321 -0.616 -1.68
##              x9   x10
## Subject1 -0.267  1.28
## Subject2 -0.098  0.32
## Subject3 -1.157  0.17
...
\hlcom{# interactions each column is standardized to mean 0 and sd 1}
\hlstd{X} \hlkwb{<-} \hlkwd{model.matrix}\hlstd{(}\hlkwd{as.formula}\hlstd{(}\hlkwd{paste0}\hlstd{(}\hlstr{"~("}\hlstd{,} \hlkwd{paste0}\hlstd{(main_effect_names,}
    \hlkwc{collapse} \hlstd{=} \hlstr{"+"}\hlstd{),} \hlstr{")^2-1"}\hlstd{)), DT} \hlopt{%>%} \hlkwd{as.data.frame}\hlstd{())} \hlopt{%>%} \hlstd{scale}
\hlkwd{crossprod}\hlstd{(X)} \hlopt{%>%} \hlstd{diag}
\end{alltt}
\begin{verbatim}
##     x1     x2     x3     x4     x5     x6     x7     x8 
##    199    199    199    199    199    199    199    199 
##     x9    x10  x1:x2  x1:x3  x1:x4  x1:x5  x1:x6  x1:x7 
##    199    199    199    199    199    199    199    199 
##  x1:x8  x1:x9 x1:x10  x2:x3  x2:x4  x2:x5  x2:x6  x2:x7 
##    199    199    199    199    199    199    199    199 
##  x2:x8  x2:x9 x2:x10  x3:x4  x3:x5  x3:x6  x3:x7  x3:x8 
##    199    199    199    199    199    199    199    199 
##  x3:x9 x3:x10  x4:x5  x4:x6  x4:x7  x4:x8  x4:x9 x4:x10 
##    199    199    199    199    199    199    199    199 
##  x5:x6  x5:x7  x5:x8  x5:x9 x5:x10  x6:x7  x6:x8  x6:x9 
##    199    199    199    199    199    199    199    199 
## x6:x10  x7:x8  x7:x9 x7:x10  x8:x9 x8:x10 x9:x10 
##    199    199    199    199    199    199    199
\end{verbatim}
\begin{alltt}
\hlkwd{apply}\hlstd{(X,} \hlnum{2}\hlstd{, mean)} \hlopt{%>%} \hlstd{sum}
\end{alltt}
\begin{verbatim}
## [1] 1.6e-16
\end{verbatim}
...
\end{verbatim}
\begin{alltt}
\hlcom{# # alternative, according to Bien et al 2013, they first}
\hlcom{# standardize the main effects, # then take all pairwise}
\hlcom{# products for interactions, then center those products #}
\hlcom{# first scale the main effects X_temp <- DT %>% scale}
\hlcom{# apply(X_temp, 2, mean) %>% sum apply(X_temp, 2, sd) %>% sum}
\hlcom{# # then create all pairwise interactions X_temp_2 <-}
\hlcom{# model.matrix(as.formula(paste0('~(',paste0(main_effect_names,}
\hlcom{# collapse = '+'),')^2-1')), X %>% as.data.frame())}
\hlcom{# apply(X_temp_2[,(p+1):ncol(X_temp_2)], 2, sd) %>% hist}
\hlcom{# apply(X_temp_2[,(p+1):ncol(X_temp_2)], 2, mean) %>% hist #}
\hlcom{# then center the product terms and combine with standardized}
\hlcom{# main effects X <- cbind(X_temp_2[,1:p],}
\hlcom{# scale(X_temp_2[,(p+1):ncol(X_temp_2)], center = T, scale =}
\hlcom{# F)) dim(X) str(X) X %>% crossprod() %>% diag}
